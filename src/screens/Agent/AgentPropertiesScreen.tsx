...
interface Property {
  ...
  project_status?: string; // New field for project status
  created_at?: string;
  created_date?: string;
  date_created?: string;
  upcoming_project_data?: any; // For nested data
}
...
const AgentPropertiesScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [statusDropdownPropertyId, setStatusDropdownPropertyId] = useState<string | number | null>(null);
  const scrollY = useRef(new Animated.Value(0)).current;

  // Project Status Options
  const PROJECT_STATUSES = [
    { label: 'Upcoming', value: 'UPCOMING', color: colors.primary },
    { label: 'Pre-Launch', value: 'PRE-LAUNCH', color: '#8B5CF6' },
    { label: 'Under Construction', value: 'UNDER CONSTRUCTION', color: '#F59E0B' },
      ...
      if (propertiesData.length > 0) {
        const fixedProperties = propertiesData.map((prop: any, index: number) => {
          ...
          return {
            ...
            project_status: prop.project_status || prop.upcoming_project_data?.project_status || 'UPCOMING',
            upcoming_project_data: prop.upcoming_project_data,
          };
        });
  ...
  const getStatusText = (status: string, isActive?: number | boolean, projectType?: string | null, projectStatus?: string) => {
    const active = isActive === 1 || isActive === true || status === 'active';
    if (projectType === 'upcoming') {
      return projectStatus ? projectStatus.toUpperCase() : 'UPCOMING';
    }
    if (active) {
      return 'ACTIVE';
    }
  ...
  const handleStatusUpdate = async (propertyId: string | number, newStatus: string) => {
    try {
      setStatusDropdownPropertyId(null); // Close dropdown
      ...
      // API Call
      const response: any = await sellerService.updateProperty(propertyId, {
        upcoming_project_data: { project_status: newStatus }
      });

      if (response && (response.success || response.status === 'success')) {
        CustomAlert.alert('Success', 'Project status updated successfully');
      } else {
        // Revert on failure
        loadProperties(false);
      ...
      <TouchableOpacity
        ...
        <View style={styles.propertyInfo}>
          <View style={styles.propertyHeader}>
            ...
            {/* Status Badge with Dropdown for Projects */}
            <View style={{ zIndex: 100 }}>
              <TouchableOpacity
                style={[
                  styles.statusBadge,
                  { backgroundColor: getStatusColor(item.status, item.is_active) },
                  item.project_type === 'upcoming' && { backgroundColor: '#E0F2FE', paddingRight: spacing.sm + 16 } // Add space for dropdown arrow
                ]}
                disabled={item.project_type !== 'upcoming'}
                onPress={() => setStatusDropdownPropertyId(statusDropdownPropertyId === item.id ? null : item.id)}
                activeOpacity={0.8}>
                <Text style={[
                  styles.statusText,
                  item.project_type === 'upcoming' && { color: '#0284C7', fontWeight: '700' }
                ]}>
                  {getStatusText(item.status, item.is_active, item.project_type, item.project_status)}
                </Text>
              ...
              {statusDropdownPropertyId === item.id && (
                <View style={[styles.statusDropdown, { zIndex: 999 }]}>
                  {PROJECT_STATUSES.map((option) => (
                    <TouchableOpacity
                      key={option.value}
                      style={[
                        styles.statusDropdownOption,
                        item.project_status === option.value && styles.statusDropdownOptionActive
                      ]}
                      onPress={() => handleStatusUpdate(item.id, option.value)}>
                      <View style={[styles.statusDot, { backgroundColor: option.color }]} />
                      <Text style={[
                        styles.statusDropdownText,
                        item.project_status === option.value && styles.statusDropdownTextActive
                      ]}>
                        {option.label}
                      </Text>
                      {item.project_status === option.value && (
