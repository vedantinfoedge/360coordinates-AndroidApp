  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.value} numberOfLines={1}>{item?.buyer_name ?? '—'}</Text>
      </View>
      {item?.buyer_phone ? (
        <TouchableOpacity style={styles.row} onPress={handlePhonePress} activeOpacity={0.7}>
          <Text style={styles.label}>Phone</Text>
          <Text style={[styles.value, styles.link]} numberOfLines={1}>{item.buyer_phone}</Text>
        </TouchableOpacity>
      ) : (
        <View style={styles.row}>
          <Text style={styles.label}>Phone</Text>
      ...
      {item?.buyer_email ? (
        <TouchableOpacity style={styles.row} onPress={handleEmailPress} activeOpacity={0.7}>
          ...
          <Text style={[styles.value, styles.link]} numberOfLines={1}>{item.buyer_email}</Text>
        </TouchableOpacity>
      ) : (
        <View style={styles.row}>
          <Text style={styles.label}>Email</Text>
...
const LeadsScreen: React.FC<Props> = ({navigation}) => {
  ...
  const keyExtractor = useCallback((item: Lead, index: number) => `${item?.created_at ?? index}-${index}`, []);
    ...
    <View style={styles.container}>
      ...
      {loading && leads.length === 0 ? (
        ...
        <View style={[styles.center, {paddingTop: contentTopPadding}]}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={() => { setLoading(true); loadLeads(); }} activeOpacity={0.8}>
